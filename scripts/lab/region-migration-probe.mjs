// Probe E (repo scripts/lab, deleted after): region-hosting + full migration.
// One fresh account: pin US -> E1 roll on devnet-us (region where it lives)
// -> undelegate ON devnet-us -> re-delegate to AS -> E2 roll on devnet-as.
...
(async () => {
  ...
  console.log('\n=== E1: roll US-pinned account ON devnet-us ===');
  await rollAndWait(US_URL, player, pda, 'E1-US');

  console.log('\n=== migrate: undelegate ON devnet-us ===');
  const usConn = createConnection(US_URL, 'processed');
  const usProv = new AnchorProvider(usConn, mkWallet(player), { commitment: 'processed', skipPreflight: true });
  ...
  try {
    const utx = await usProg.methods.undelegate()
      ...
      .transaction();
    utx.feePayer = player.publicKey;
    const usig = await sendMagicTx(usConn, utx, [player], { skipPreflight: true });
    await usConn.confirmTransaction({ signature: usig }, 'processed');
    console.log('  [undelegate ON US] OK', usig);
    await sleep(5000);
  } catch (e) {
    console.log(`  [undelegate ON US] FAILED: ${(e.message || '').slice(0, 200)}`);
  }
  console.log('  after undelegate:', JSON.stringify(await getDelegationStatus(baseConn, pda).catch(() => null)));

  console.log('\n=== re-delegate to AS ===');
  try {
    tx = await baseProg.methods.delegate()
      .accounts({ payer: sponsor.publicKey, playerAuthority: player.publicKey, player: pda, bufferPlayer: buffer, delegationRecordPlayer: record, delegationMetadataPlayer: metadata, ownerProgram: PROGRAM_ID, delegationProgram: DELEGATION_PROGRAM, systemProgram: SystemProgram.programId })
      .remainingAccounts([{ pubkey: V_AS, isSigner: false, isWritable: false }])
      .transaction();
    tx.feePayer = sponsor.publicKey;
    const sig3 = await sendMagicTx(baseConn, tx, [sponsor], { skipPreflight: true });
    await baseConn.confirmTransaction({ signature: sig3 }, 'confirmed');
    console.log('  [re-delegate->AS] OK');
  } catch (e) {
    console.log(`  [re-delegate->AS] FAILED: ${(e.message || '').slice(0, 200)}`);
  }
  await sleep(3000);
  console.log('  final status:', JSON.stringify(await getDelegationStatus(baseConn, pda).catch(() => null)));

  await sleep(2000);
  console.log('\n=== E2: roll AS-pinned account ON devnet-as ===');
  await rollAndWait(AS_URL, player, pda, 'E2-AS');
