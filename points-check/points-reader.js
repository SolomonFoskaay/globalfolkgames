// Points viewer: raw on-chain M3/M4 read, mirroring dashboard/recovery.html
// fetchM3/fetchM4 byte-for-byte. NO Anchor typed fetch, NO cache, no wallet
// signing needed (a points ledger is public data derived from the wallet).
//
// Region order matches the client + recovery: healthy ER regions first (AS,
// EU), US LAST as a legacy fallback (devnet-us is the banned/throttled one),
// then the base RPC as recovery's final fallback.
...
//   found: { found:true, data, url }
//   absent with no RPC error  -> { found:false, absent:true  (all regions
//     answered healthy but the account bytes were not there) }
//   at least one region errored -> { found:false, errors:[url,...] } plus the
//     healthy-but-empty regions in `answered`.
...
const $wallet = document.getElementById('pv-wallet');
const $results = document.getElementById('pv-results');

function signedInWallet() {
  // Log which source supplied the wallet, so a wrong-wallet grabbed from the
  // site menu is caught instantly (the failing step shows here).
  try {
  ...
  console.warn('[points-reader] No signed-in wallet: getDynamicSolanaWallet() returned nothing and currentProfile.solana_wallet is unset');
  return null;
}
...
async function readWallet(wallet) {
  ...
  // FINAL DIAGNOSIS: consolidate every step into a verdict so the failing
  // step (if any) is spelled out, not guessed.
...
function render({ wallet, m3Rows, m4, gaddr, gres, diagnosis, elapsedMs }) {
  ...
  for (const g of m3Rows) {
    if (!g.from) {
      // g.from holds the not-found result: absent (healthy, no bytes) vs errors.
...
document.getElementById('pv-go').addEventListener('click', () => {
  const w = $wallet.value.trim();
  if (!w) return $results.innerHTML = '<div class="pv-empty">Enter a wallet address first.</div>';
  if (!/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(w)) {
    return $results.innerHTML = '<div class="pv-empty">That does not look like a Solana wallet address.</div>';
  }
  readWallet(w);
});
document.getElementById('pv-me').addEventListener('click', () => {
  const w = signedInWallet();
  if (!w) return $results.innerHTML = '<div class="pv-empty">No signed-in wallet detected. Sign in, or paste an address manually.</div>';
  $wallet.value = w;
  readWallet(w);
});
$wallet.addEventListener('keydown', (e) => { if (e.key === 'Enter') document.getElementById('pv-go').click(); });