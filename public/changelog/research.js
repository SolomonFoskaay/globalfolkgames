...
//   1. OVERVIEW (/changelog/research.html): the list of research items.
//   2. ITEM (/changelog/research.html?r=arc): one item's full write-up.
//
...
(function () {
  const RESEARCH_JSON = '/changelog/research.json';
  let cache = null;
    ...
    return `
      <article class="entry roadmap-entry econ-entry research-entry" style="margin-bottom:16px;">
        <div class="entry-head">
          <span class="entry-version" style="font-size:0.9rem;">research/${esc(item.id)}</span>
  ...
  function overviewHtml(data) {
    ...
    const cards = (data.items || []).map((item) => `
      <a class="research-card" href="/changelog/research.html?r=${encodeURIComponent(item.id)}">
  ...
  async function renderResearch() {
    const host = document.getElementById('research-items');
    if (!host) return;
    try {
      cache = cache || await loadJson(RESEARCH_JSON);
    } catch (e) {
    ...
    if (id && !item) {
      host.innerHTML = '<p class="empty">Unknown research id "' + esc(id) + '". <a href="/changelog/research.html" style="color:#f87818;">Back to all research</a>.</p>';
      return;
    }
    if (item) {
      host.innerHTML = `<p style="margin:0 0 12px;"><a href="/changelog/research.html" style="color:#f87818;">← All research</a></p>` + itemHtml(item, cache.statusLegend);
