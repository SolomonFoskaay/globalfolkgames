        ...
        if (isUserSeat && window.magicblockDice && window.magicblockDice.available()) {
            try {
                ...
                if (Array.isArray(v) && v.length === 2) {
                    ...
                    // always 404. The roll is verified honestly instead: it
                    // happened on-chain via the ER VRF, and the base-layer tx
                    // that created+delegated this player's dice account IS
                    // devnet-visible, so that is what we link.
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (data && Number.isInteger(data.roll1) && Number.isInteger(data.roll2)) {
                        ...
                        // sponsored + delegated server-side, and its rolls run
                        // on the same ER queue.
...
function finalizeDiceScores() {
    ...
    setTimeout(() => {
        ...
        if (!hasAnyValidMoveForCurrentTurn()) {
            ...
            // Dice cleared: redraw so the settled dice vanish and, for a human
            // seat with valid moves, movable tokens resume blinking.
