...
// ------------------------------------------------------------------ DEMO ADD
// Foskaay GGI Ludo demo: tumble two dice then LOCK each onto a KNOWN on-chain
// value (the contract's dice), using the exact same physics + CSS-3D cubes as
// ludo-lab. `showRemoteDice` (above) shows the settled faces instantly; this one
// gives the roll its real bounce first. The value shown is always the contract's.
window.showDiceTumble = function (die1, die2) {
    try {
        displayDiceOnBoard = true;
        isDiceRolled = true;
        physicalDice = [
            { x: 150, y: 150, vx: 11 + Math.random() * 4, vy: 8 + Math.random() * 3, value: 1, finalValue: die1, q: null },
            { x: 360, y: 190, vx: -(10 + Math.random() * 4), vy: 9 + Math.random() * 3, value: 1, finalValue: die2, q: null }
        ];
        if (typeof playDiceRattle === 'function') { try { playDiceRattle(); } catch (e) {} }
        if (typeof buildDice3dStage === 'function') { try { buildDice3dStage(); } catch (e) {} }
        var d1 = document.getElementById('val-d1');
        var d2 = document.getElementById('val-d2');
        var tt = document.getElementById('val-total');
        if (d1) d1.innerText = die1;
        if (d2) d2.innerText = die2;
        if (tt) tt.innerText = 'Total: ' + (die1 + die2);
        if (typeof runDicePhysicsCalculations === 'function') runDicePhysicsCalculations();
    } catch (e) { /* soft */ }
};
